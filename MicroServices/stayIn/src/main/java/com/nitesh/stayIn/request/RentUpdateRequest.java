...
//	@Pattern(regexp = "^(true|false)$", message = "restartable field allowed input: true or false")
	...
	public RentUpdateRequest(Integer userId, Integer rentId,
			@NotBlank(message = "Address not be blank.") @NotNull(message = "Address must be required!") String rentAddress,
			@NotBlank(message = "City not be blank.") @NotNull(message = "City must be required!") String rentCity,
			@NotBlank(message = "State not be blank.") @NotNull(message = "State must be required!") String rentState,
			@NotBlank(message = "ContactNo not be blank.") @NotNull(message = "ContactNo must be required!") String rentContactNo,
			@NotBlank(message = "Rent Type not be blank.") @NotNull(message = "Rent Type must be required!") String rentType,
			@Positive(message = "Build Up Area must be have positive value") Float builtUpArea,
			@Min(value = 1, message = "Minimum value of age of property is 1") @NotNull(message = "Age of Property can not be null") @Positive(message = "Minimum value of age of property is 1") int ageOfProperty,
			@PositiveOrZero(message = "Property On Floor is either 0 or up") int propertyOnFloor,
			@Positive(message = "Total Floor must be 1 or above") int totalFloor,
			@Positive(message = "Minimum value of BHK is 1") int bhk,
			@PositiveOrZero(message = "Property On Floor is either 0 or up") int bathrooms,
			@Positive(message = "Minimum value of fixCharge is 1") int fixCharge,
			@Positive(message = "Minimum value of max person capacity is 1.") int maxPerson, boolean powerBackUp,
			@NotBlank(message = "Furnishing not be blank.") @NotNull(message = "Furnishing must be required!") String furnishing,
			@NotBlank(message = "Properties Details not be blank.") @NotNull(message = "Properties Details must be required!") String aboutThisProperty,
			@NotBlank(message = "Term & Condition not be blank.") @NotNull(message = "Term & Condition must be required!") String termAndConditions,
			Set<String> amenities, List<MultipartFile> file) {
		this.userId = userId;
		this.rentId = rentId;
	...
	public String toString() {
		return "RentUpdateRequest [userId=" + userId + ", rentId=" + rentId + ", rentAddress=" + rentAddress
				+ ", rentCity=" + rentCity + ", rentState=" + rentState + ", rentContactNo=" + rentContactNo
				+ ", rentType=" + rentType + ", builtUpArea=" + builtUpArea + ", ageOfProperty=" + ageOfProperty
				+ ", propertyOnFloor=" + propertyOnFloor + ", totalFloor=" + totalFloor + ", bhk=" + bhk
				+ ", bathrooms=" + bathrooms + ", fixCharge=" + fixCharge + ", maxPerson=" + maxPerson
				+ ", powerBackUp=" + powerBackUp + ", furnishing=" + furnishing + ", aboutThisProperty="
				+ aboutThisProperty + ", termAndConditions=" + termAndConditions + ", amenities=" + amenities
				+ ", file=" + file + "]";
	}
